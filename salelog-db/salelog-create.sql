...
CREATE TABLE `order` (
  ...
  `call_girl_code` VARCHAR(255) NOT NULL,
  ...
  KEY `key_call_girl_code` (`call_girl_code`),
  KEY `key_begin_date` (`begin_date`),
  KEY `key_end_date` (`end_date`),
  CONSTRAINT `fk_order__call_girl` FOREIGN KEY (`call_girl_code`) REFERENCES `call_girl` (`code`),
...
CREATE TABLE `product_sale` (
  ...
  `end_date` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY  (`pk_id`),
  UNIQUE KEY `unique_object_id__begin_date` (`order_id`,`product_code`,`begin_date`),
  ...
  CONSTRAINT `fk_product_sale__product_code` FOREIGN KEY (`product_code`) REFERENCES `product` (`code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
